...
def draw_page(page, stream):
    ...
    draw_background(
        stream,
        stacking_context.box.background,
        clip_box=False,
        bleed=page.bleed,
        marks=marks,
    )
...
def draw_stacking_context(stream, stacking_context):
    ...
    with stacked(stream):
        ...
        with stacked(stream):
            ...
            clip = box.style["overflow"] != "visible" and not isinstance(
                box, boxes.PageBox
            )
            if clip:
        ...
        for row in row_group.children:
            ...
            for cell in row.children:
                draw_cell_background = (
                    ...
                    or cell.style["empty_cells"] == "show"
                    or not cell.empty
                )
...
def draw_collapsed_borders(stream, table):
    ...
    row_heights = [
        row.height for row_group in table.children for row in row_group.children
    ]
    ...
    if not (row_heights and column_widths):
        ...
        return
    row_positions = [
        row.position_y for row_group in table.children for row in row_group.children
    ]
...
def draw_inline_level(
    stream, page, box, offset_x=0, text_overflow="clip", block_ellipsis="none"
):
