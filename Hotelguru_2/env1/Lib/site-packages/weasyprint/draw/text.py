...
def draw_text(stream, textbox, offset_x, text_overflow, block_ellipsis):
    ...
    if "overline" in text_decoration_values:
        offset_y = textbox.baseline - textbox.pango_layout.ascent + thickness / 2
...
def draw_first_line(stream, textbox, text_overflow, block_ellipsis, matrix):
    ...
    if block_ellipsis != "none":
        while index:
            last_word_end = get_last_word_end(
                textbox.pango_layout.text[: -len(ellipsis)], textbox.style["lang"]
            )
    ...
    while run != ffi.NULL:
        ...
        for i in range(num_glyphs):
            ...
            if (
                glyph == pango.PANGO_GLYPH_EMPTY
                or glyph & pango.PANGO_GLYPH_UNKNOWN_FLAG
            ):
...
def draw_text_decoration(stream, textbox, offset_x, offset_y, thickness, color):
    ...
    draw_line(
        ...
        textbox.position_y + offset_y,
        thickness,
        textbox.style["text_decoration_style"],
        color,
        offset_x,
    )
