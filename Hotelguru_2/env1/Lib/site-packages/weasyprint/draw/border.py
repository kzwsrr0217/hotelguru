...
def set_mask_border(stream, box):
    ...
    draw_border_image(
        box,
        mask_stream,
        box.mask_border_image,
        box.style["mask_border_slice"],
        box.style["mask_border_repeat"],
        box.style["mask_border_outset"],
...
def draw_border(stream, box):
    ...
    # Draw column borders.
    columns = isinstance(box, boxes.BlockContainerBox) and (
        box.style["column_width"] != "auto" or box.style["column_count"] != "auto"
    )
    ...
    if box.style["border_image_source"][0] != "none" and box.border_image is not None:
        draw_border_image(
            ...
            stream,
            box.border_image,
            box.style["border_image_slice"],
            box.style["border_image_repeat"],
            box.style["border_image_outset"],
...
def draw_border_image(
    box, stream, image, border_slice, border_repeat, border_outset, border_width
):
    ...
    intrinsic_width, intrinsic_height = replaced.default_image_sizing(
        ...
        default_width=box.border_width(),
        default_height=box.border_height(),
    )
    ...
    def compute_width_adjustment(dimension, original, intrinsic, area_dimension):
        if dimension == "auto":
            return intrinsic
    ...
    border_top = compute_width_adjustment(border_width[0], border_top, slice_top, h)
    ...
    border_left = compute_width_adjustment(border_width[3], border_left, slice_left, w)

    def draw_border_image_region(
        x,
        y,
        width,
        height,
        ...
        if 0 in (intrinsic_width, width, slice_width):
            ...
            if repeat_x == "repeat":
                ...
                extra_dx = (width / scale_x - n_repeats_x * slice_width) / (
                    n_repeats_x + 1
                )
            elif repeat_x == "round":
        ...
        if 0 in (intrinsic_height, height, slice_height):
            ...
            if repeat_y == "repeat":
                ...
                # Space is before the first repeat and after the last,
                # so there's one more space than repeat.
                extra_dy = (height / scale_y - n_repeats_y * slice_height) / (
                    n_repeats_y + 1
                )
            elif repeat_y == "round":
    ...
    if should_fill and x_middle and y_middle:
        ...
        draw_border_image_region(
            ...
            repeat_x=repeat_x,
            repeat_y=repeat_y,
            scale_x=scale_left or scale_right,
            scale_y=scale_top or scale_bottom,
        )


def clip_border_segment(
    stream, style, width, side, border_box, border_widths=None, radii=None
):
    ...
    if style in ("dotted", "dashed"):
        ...
        if rounded1 or rounded2:
            ...
            def draw_dots(dashes, line, way, x, y, px, py, chl):
                ...
                for i in range(0, dashes, 2):
                    ...
                    if side in ("top", "bottom"):
                        ...
                        stream.line_to(x + px - way * px * 1 / tan(angle2), main_offset)
                        stream.line_to(x + px - way * px * 1 / tan(angle1), main_offset)
                    elif side in ("left", "right"):
                        stream.move_to(main_offset + px, y + py)
                        stream.line_to(main_offset, y + py + way * py * tan(angle2))
                        stream.line_to(main_offset, y + py + way * py * tan(angle1))
            ...
            line, offset = draw_dots(dashes1, line, way, bbx, bby, px1, py1, chl1)
            line = draw_dots(dashes2, line, -way, bbx + bbw, bby + bbh, px2, py2, chl2)[
                0
            ]
            ...
            for i in range(0, round(length / dash), 2):
                if side == "top":
                    ...
                    stream.rectangle(bbx + bbw - width, bby + i * dash, width, dash)
                elif side == "bottom":
                    stream.rectangle(bbx + i * dash, bby + bbh - width, dash, width)
                elif side == "left":
...
def draw_rect_border(stream, box, widths, style, color):
    ...
    if style in ("ridge", "groove"):
        ...
        stream.rectangle(
            bbx + bl / 2, bby + bt / 2, bbw - (bl + br) / 2, bbh - (bt + bb) / 2
        )
        stream.fill(even_odd=True)
        stream.rectangle(
            bbx + bl / 2, bby + bt / 2, bbw - (bl + br) / 2, bbh - (bt + bb) / 2
    ...
    if style == "double":
        stream.rectangle(
            bbx + bl / 3, bby + bt / 3, bbw - (bl + br) / 3, bbh - (bt + bb) / 3
        )
        stream.rectangle(
            bbx + bl * 2 / 3,
            bby + bt * 2 / 3,
...
def rounded_box(stream, radii):
    ...
    stream.curve_to(x + w - tr[0] * r, y, x + w, y + tr[1] * r, x + w, y + tr[1])
    stream.line_to(x + w, y + h - br[1])
    stream.curve_to(
        x + w, y + h - br[1] * r, x + w - br[0] * r, y + h, x + w - br[0], y + h
    )
    stream.line_to(x + bl[0], y + h)
    stream.curve_to(x + bl[0] * r, y + h, x, y + h - bl[1] * r, x, y + h - bl[1])
    stream.line_to(x, y + tl[1])
    stream.curve_to(x, y + tl[1] * r, x + tl[0] * r, y, x + tl[0], y)
