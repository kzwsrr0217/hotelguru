...
HTML5_UA_STYLESHEET = CSS(string=HTML5_UA, counter_style=HTML5_UA_COUNTER_STYLE)
...
def make_replaced_box(element, box, image):
    ...
    """
    type_ = (
        boxes.BlockReplacedBox
        if "block" in box.style["display"]
...
def handle_img(element, box, get_image_from_uri, base_url):
    ...
    alt = element.get("alt")
    if src:
        image = get_image_from_uri(url=src, orientation=box.style["image_orientation"])
...
def handle_embed(element, box, get_image_from_uri, base_url):
    ...
    if src:
        image = get_image_from_uri(
            url=src, forced_mime_type=type_, orientation=box.style["image_orientation"]
        )
        if image is not None:
            return [make_replaced_box(element, box, image)]
    # No fallback.
...
def handle_object(element, box, get_image_from_uri, base_url):
    ...
    if data:
        image = get_image_from_uri(
            url=data, forced_mime_type=type_, orientation=box.style["image_orientation"]
        )
        if image is not None:
            return [make_replaced_box(element, box, image)]
...
def handle_colgroup(element, box, _get_image_from_uri, _base_url):
    ...
    if isinstance(box, boxes.TableColumnGroupBox):
        if not any(child.tag == "col" for child in element):
            box.children = [
                boxes.TableColumnBox.anonymous_from(box, []) for _ in range(box.span)
            ]
    return [box]
    ...
    for element in html.wrapper_element.query_all("title", "meta", "link"):
        ...
        if element.tag == "title" and title is None:
            ...
            if name == "keywords":
                ...
                custom[name] = content
        elif element.tag == "link" and element_has_link_type(element, "attachment"):
            ...
            if url is None:
                ...
                attachment = Attachment(
                    url=url, description=attachment_title, url_fetcher=html.url_fetcher
                )
