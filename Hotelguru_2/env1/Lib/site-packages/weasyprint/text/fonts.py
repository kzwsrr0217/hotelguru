...
_check_font_configuration(
    ffi.gc(fontconfig.FcInitLoadConfigAndFonts(), fontconfig.FcConfigDestroy)
)
...
def get_font_description(style):
    ...
    if style["font_variation_settings"] != "normal":
        string = ",".join(
            f"{key}={value}" for key, value in style["font_variation_settings"]
        ).encode()
