...
class Layout:
    ...
    def setup(self, context, style):
        ...
        self.layout = ffi.gc(
            pango.pango_layout_new(pango_context), gobject.g_object_unref
        )
        ...
        if text_decoration != "none":
            ...
            self.ascent = FROM_UNITS * (pango.pango_font_metrics_get_ascent(metrics))
...
def split_first_line(
    ...
    if dictionary_iterations:
        for first_word_part in dictionary_iterations:
            new_first_line_text = (
                first_line_text + second_line_text[:start_word] + first_word_part
            )
    ...
    can_break = style["word_break"] == "break-all" or (
        is_line_start
        and (
            overflow_wrap == "anywhere"
            or (overflow_wrap == "break-word" and not minimum)
        )
    )
