...
def build_formatting_structure(
    element_tree,
    style_for,
    get_image_from_uri,
    base_url,
...
def element_to_box(
    ...
    if style["float"] == "footnote":
        ...
        marker = make_box(f"{element.tag}::footnote-marker", marker_style, [], element)
...
def marker_to_box(
    ...
    if parent_style["list_style_position"] == "outside":
        ...
        marker_box.style["transform"] = (("translate", (translate_x, translate_y)),)
    else:
        marker_box = boxes.InlineBox.anonymous_from(box, children)
    yield marker_box


def compute_content_list(
    content_list,
    parent_box,
    counter_values,
    css_token,
    ...
    def add_text(text):
        ...
        if text:
            if content_boxes and isinstance(content_boxes[-1], boxes.TextBox):
                ...
                content_boxes.append(boxes.TextBox.anonymous_from(parent_box, text))
    ...
    for type_, value in content_list:
        if type_ == "string":
            ...
            if not in_page_context:
                ...
                LOGGER.warning(
                    '"string(%s)" is only allowed in page margins', " ".join(value)
                )
                continue
            ...
            if not in_page_context:
                LOGGER.warning(
                    '"element(%s)" is only allowed in page margins', " ".join(value)
                )
                continue
            ...
            leader_box = boxes.InlineBox.anonymous_from(parent_box, (text_box,))
...
def content_to_boxes(
    ...
    def parse_again(mixin_pagebased_counters=None):
        ...
        if len(parent_box.children) == 1 and isinstance(
            parent_box.children[0], boxes.LineBox
        ):
...
def compute_string_set(
    ...
    if box_list is not None:
        string = "".join(box.text for box in box_list if isinstance(box, boxes.TextBox))
...
def compute_bookmark_label(
    element, box, content_list, counter_values, target_collector, counter_style
):
    ...
    def parse_again(mixin_pagebased_counters={}):
        ...
        local_counters.update(box.cached_counter_values)
        compute_bookmark_label(
            element, box, content_list, local_counters, target_collector, counter_style
        )
...
def table_boxes_children(box, children):
    ...
    if isinstance(box, boxes.TableColumnBox):  # rule 1.1
        ...
        children = [
            child for child in children if isinstance(child, boxes.TableColumnBox)
        ]
        ...
        if not children:
            if box.span is None or box.span < 1:
                ...
                span = box.span
            children = [
                boxes.TableColumnBox.anonymous_from(box, []) for _ in range(span)
            ]
    ...
    children = [
        ...
        for prev_child, child, next_child in zip(
            [None] + children[:-1], children, children[1:] + [None]
        )
        if not (
            # Ignore some whitespace: rule 1.4
            prev_child
            and prev_child.internal_table_or_caption
            and next_child
            and next_child.internal_table_or_caption
            and is_whitespace(child)
        )
    ]
    ...
    if isinstance(box, boxes.TableBox):
        ...
        children = wrap_improper(
            box, children, boxes.TableRowBox, lambda child: child.proper_table_child
        )
    ...
    if isinstance(box, boxes.InlineBox):
        ...
        children = wrap_improper(
            ...
            boxes.TableBox,
            lambda child: (
                not child.proper_table_child or parent_type in child.proper_parents
            ),
        )
...
def wrap_table(box, children):
    ...
    column_groups = list(wrap_improper(box, columns, boxes.TableColumnGroupBox))
    grid_x = 0
...
def process_whitespace(box, following_collapsible_space=False):
    ...
    if isinstance(box, boxes.TextBox):
        ...
        space_collapse = box.style["white_space"] in ("normal", "nowrap", "pre-line")
...
def inline_in_block(box):
    ...
    if box_children and box.leading_collapsible_space is False:
        box.leading_collapsible_space = box_children[0].leading_collapsible_space
    ...
    for child_box in children:
        ...
        if new_line_children and child_box.is_absolutely_positioned():
            ...
            if new_line_children or not (
                isinstance(child_box, boxes.TextBox)
                and
                # Sequence of white-space was collapsed to a single
                # space by process_whitespace().
                child_box.text == " "
                and child_box.style["white_space"] in ("normal", "nowrap", "pre-line")
            ):
                new_line_children.append(child_box)
        else:
...
def block_in_inline(box):
    ...
    for child in box.children:
        if isinstance(child, boxes.LineBox):
            ...
            stack = None
            while True:
                new_line, block, stack = _inner_block_in_inline(child, skip_stack=stack)
                if block is None:
                    break
...
def _inner_block_in_inline(box, skip_stack=None):
    ...
    for i, child in enumerate(box.children[skip:]):
        index = i + skip
        if isinstance(child, boxes.BlockLevelBox) and child.is_in_normal_flow():
...
def set_viewport_overflow(root_box):
    ...
    chosen_box = root_box
    if (
        root_box.element_tag.lower() == "html"
        and root_box.style["overflow"] == "visible"
    ):
...
def box_text(box):
    if isinstance(box, boxes.TextBox):
        ...
        return "".join(
            ...
            if not child.element_tag.endswith("::before")
            and not child.element_tag.endswith("::after")
            and not child.element_tag.endswith("::marker")
            and isinstance(child, boxes.TextBox)
        )
    return ""
    ...
    if text_part in ("text", "content"):
        ...
        if isinstance(box, boxes.ParentBox):
            return "".join(
                box_text(child)
                for child in box.descendants()
                if child.element_tag.endswith(f"::{text_part}")
