...
class Box:
    ...
    def is_in_normal_flow(self):
        ...
        return not (
            self.is_floated()
            or self.is_absolutely_positioned()
            or self.is_running()
            or self.is_footnote()
        )
    ...
    def is_monolithic(self):
        ...
        return (
            isinstance(self, AtomicInlineLevelBox)
            or isinstance(self, ReplacedBox)
            or self.style["overflow"] in ("auto", "scroll")
            or (self.style["overflow"] == "hidden" and self.style["height"] != "auto")
        )
    ...
    def establishes_formatting_context(self):
        ...
        return (
            ...
            or self.is_absolutely_positioned()
            or self.is_column
            or (isinstance(self, BlockContainerBox) and not isinstance(self, BlockBox))
            or (isinstance(self, BlockBox) and self.style["overflow"] != "visible")
            or "flow-root" in self.style["display"]
        )
