...
def get_use_tree(svg, node, font_size):
    ...
    if svg_url.scheme == "data":
        svg_url = parse_url("")
    same_origin = parsed_url[:3] == ("", "", "") or parsed_url[:3] == svg_url[:3]
...
def draw_gradient_or_pattern(svg, node, name, font_size, opacity, stroke):
    ...
    if name in svg.gradients:
        return draw_gradient(svg, node, svg.gradients[name], font_size, opacity, stroke)
    elif name in svg.patterns:
        return draw_pattern(svg, node, svg.patterns[name], font_size, opacity, stroke)
...
def draw_gradient(svg, node, gradient, font_size, opacity, stroke):
    ...
    if "gradientTransform" in gradient.attrib:
        transform_matrix = transform(
            gradient.get("gradientTransform"), font_size, svg.normalized_diagonal
        )
    ...
    if gradient.tag == "linearGradient":
        ...
        positions, colors, coords = spread_radial_gradient(
            spread, positions, colors, fx, fy, fr, cx, cy, r, width, height, matrix
        )

    alphas = [color[3] for color in colors]
    alpha_couples = [(alphas[i], alphas[i + 1]) for i in range(len(alphas) - 1)]
    color_couples = [
        [colors[i][:3], colors[i + 1][:3], 1] for i in range(len(colors) - 1)
    ]
    ...
    function = group.create_stitching_function(domain, encode, bounds, sub_functions)
    shading = group.add_shading(shading_type, "RGB", domain, coords, extend, function)
...
def spread_radial_gradient(
    ...
    if spread in ("repeat", "reflect"):
        ...
        max_distance = hypot(max(abs(fx), abs(tw - fx)), max(abs(fy), abs(th - fy)))
        ...
        if repeat_after > 0:
            ...
            positions = [i + position for i in range(repeat) for position in positions]
        ...
        if full_repeat:
            ...
            if spread == "repeat":
                ...
                for i in range(full_repeat):
                    colors += original_colors[:: -1 if (i + repeat_after) % 2 else 1]
            positions = [
                i - full_repeat + position
                for i in range(full_repeat)
        ...
        for i, position in enumerate(reverse, start=1):
            if position == ratio:
                ...
                new_positions = [
                    position - full_repeat - 1 for position in original_positions[-i:]
                ]
            ...
            if position < ratio:
                ...
                new_positions = [
                    position - 1 - full_repeat
                    for position in original_positions[-(i - 1) :]
                ]
...
def draw_pattern(svg, node, pattern, font_size, opacity, stroke):
    ...
    if pattern.get("patternUnits") == "userSpaceOnUse":
        ...
        pattern_width = size(pattern.attrib.pop("width", "1"), font_size, 1) * width
        pattern_height = size(pattern.attrib.pop("height", "1"), font_size, 1) * height
    ...
    stream_pattern = svg.stream.add_pattern(
        0, 0, pattern_width, pattern_height, pattern_width, pattern_height, matrix
    )
    stream_pattern.set_alpha(opacity)

    group = stream_pattern.add_group(0, 0, pattern_width, pattern_height)
    Pattern(pattern, svg).draw(
        group, pattern_width, pattern_height, svg.base_url, svg.url_fetcher, svg.context
    )
    ...
    for child in filter_node:
        if child.tag == "feOffset":
            if filter_node.get("primitiveUnits") == "objectBoundingBox":
                ...
                dx, dy = svg.point(child.get("dx", 0), child.get("dy", 0), font_size)
...
def paint_mask(svg, node, mask, font_size):
    ...
    mask.attrib["height"] = size(mask.get("height", "120%"), font_size, height_ref)
    mask.attrib["width"] = size(mask.get("width", "120%"), font_size, width_ref)
