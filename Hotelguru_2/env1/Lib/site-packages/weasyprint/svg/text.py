...
def text(svg, node, font_size):
    ...
    style["font_family"] = [
        font.strip("\"'") for font in node.get("font-family", "sans-serif").split(",")
    ]
    ...
    if "x" in node.attrib:
        x = [
            size(i, font_size, svg.inner_width)
            for i in normalize(node.attrib["x"]).strip().split(" ")
        ]
    if "y" in node.attrib:
        y = [
            size(i, font_size, svg.inner_height)
    ...
    alignment_baseline = node.get("dominant-baseline", node.get("alignment-baseline"))
    ...
    for i, ((x, y, dx, dy, r), letter) in enumerate(letters_positions):
        ...
        points = ((x_position, y_position), (x_position + width, y_position - height))
