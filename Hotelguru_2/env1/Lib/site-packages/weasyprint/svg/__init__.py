...
class Node:
    ...
    def cascade(self, child):
        ...
        if child.tag in ("text", "textPath", "a"):
            ...
            child._wrapper.etree_children = [child._etree_node for child in children]
    ...
    def text_children(self, element, trailing_space, text_root=False):
        ...
        original_rotate = [
            float(i) for i in normalize(self.get("rotate")).strip().split(" ") if i
        ]
    ...
    def override_iter(self, iterator):
        ...
        self.__class__ = type("Node", (Node,), {"__iter__": lambda _: iterator})
...
class SVG:
    ...
    def draw(
        self, stream, concrete_width, concrete_height, base_url, url_fetcher, context
    ):
    ...
    def draw_markers(self, node, font_size, fill_stroke):
        ...
        while node.vertices:
            ...
            translate_x, translate_y = self.point(
                marker_node.get("refX"), marker_node.get("refY"), font_size
            )
            ...
            if "viewBox" in marker_node.attrib:
                ...
                align = marker_node.get("preserveAspectRatio", "xMidYMid").split(" ")[0]
                if align == "none":
                ...
                if y_position == "mid":
                    clip_y += (viewbox_height - marker_height / scale_y) / 2
                elif y_position == "max":
                ...
                clip_box = (
                    clip_x,
                    clip_y,
                    marker_width / scale_x,
                    marker_height / scale_y,
                )
            else:
                scale_x = scale_y = 1
    ...
    def fill_stroke(self, node, font_size, text=False):
        ...
        dash_array = tuple(
            ...
            for value in normalize(node.get("stroke-dasharray")).split()
            if value != "none"
        )
        dash_condition = (
            dash_array
            and not all(value == 0 for value in dash_array)
            and not any(value < 0 for value in dash_array)
        )
        if dash_condition:
