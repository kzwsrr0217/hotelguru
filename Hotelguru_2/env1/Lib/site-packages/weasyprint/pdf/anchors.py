...
def add_outlines(pdf, bookmarks, parent=None):
    ...
    for title, (page, x, y), children, state in bookmarks:
        ...
        outline = pydyf.Dictionary({"Title": pydyf.String(title), "Dest": destination})
        pdf.add_object(outline)
        children_outlines, children_count = add_outlines(pdf, children, parent=outline)
...
def add_forms(forms, matrix, pdf, page, resources, stream, font_map):
    ...
    context = ffi.gc(
        pango.pango_font_map_create_context(font_map), gobject.g_object_unref
    )
    inputs_with_forms = [
    ...
    for i, (form, element, style, rectangle) in enumerate(inputs_with_forms):
        ...
        if input_type in ("radio", "checkbox"):
            ...
            if "multiple" in element.attrib:
                ...
                field["Ff"] = 1 << (18 - 1)
                field["V"] = (
                    selected_values[-1] if selected_values else pydyf.String("")
                )
            pdf.add_object(field)
...
def add_annotations(links, matrix, document, pdf, page, annot_files, compress):
    ...
    for link_type, annot_target, rectangle, _ in links:
        ...
        if annot_target not in annot_files:
            ...
            attachment = Attachment(url=annot_target, url_fetcher=document.url_fetcher)
            annot_files[annot_target] = write_pdf_attachment(pdf, attachment, compress)
    ...
    for page in pages:
        ...
        for link in page.links:
            ...
            if link_type == "internal":
                if anchor_name not in anchors:
                    LOGGER.error(
                        "No anchor #%s for internal URI reference", anchor_name
                    )
                else:
