...
VARIANTS = {
    name: data
    for variants in (pdfa.VARIANTS, pdfua.VARIANTS, debug.VARIANTS)
...
def generate_pdf(document, target, zoom, **options):
    ...
    for page_number, (page, links_and_anchors) in enumerate(
        ...
        page_width = scale * (page.width + page.bleed["left"] + page.bleed["right"])
        page_height = scale * (page.height + page.bleed["top"] + page.bleed["bottom"])
        ...
        add_annotations(
            links_and_anchors[0], matrix, document, pdf, pdf_page, annot_files, compress
        )
        add_forms(
            page.forms,
            matrix,
    ...
    if options["attachments"]:
        for attachment in options["attachments"]:
            if not isinstance(attachment, Attachment):
                attachment = Attachment(attachment, url_fetcher=document.url_fetcher)
    ...
    if variant:
        variant_function(pdf, metadata, document, page_streams, attachments, compress)
