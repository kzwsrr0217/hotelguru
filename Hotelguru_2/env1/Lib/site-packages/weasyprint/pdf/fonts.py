...
class Font:
    def __init__(self, pango_font, description, font_size):
        ...
        if (width := self.variations.get("wdth")) is not None:
            stretch = min(
                PANGO_STRETCH_PERCENT.items(), key=lambda item: abs(item[0] - width)
            )[1]
        ...
        self.hash = "".join(
            chr(65 + letter % 26)
            for letter in md5(description_string, usedforsecurity=False).digest()[:6]
        )
    ...
    def _harfbuzz_subset(self, cmap, hinting):
        ...
        drop_tables = tuple(
            harfbuzz.hb_tag_from_string(name, -1)
            for name in (
                b"BASE",
                b"DSIG",
                b"EBDT",
...
def _build_bitmap_font_dictionary(font_dictionary, pdf, font, widths, compress, subset):
    ...
    for key, glyph in font_glyphs.items():
        ...
        if 0 in (width, height) or not data:
            ...
            bitmap_bits = "".join(
                bits[i * width : (i + 1) * width] + padding * "0" for i in range(height)
            )
        ...
        if glyph_info["bitmap"] is None:
            ...
            for subglyph in glyph_info["subglyphs"]:
                ...
                for row_y in range(subglyph["height"]):
                    row_slice = slice(
                        row_y * subglyph["stride"], (row_y + 1) * subglyph["stride"]
                    )
...
def _build_vector_font_dictionary(
    font_dictionary, pdf, font, widths, compress, reference, pdf_version
):
    ...
    if str(pdf_version) <= "1.4":  # Cast for bytes and None
        ...
        for cid in cids:
            bits[cid] = "1"
        stream = pydyf.Stream(
            (int("".join(bits), 2).to_bytes(padded_width, "big"),), compress=compress
        )
