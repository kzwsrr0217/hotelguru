...
def shrink_to_fit(context, box, available_content_width):
    ...
    *Warning:* both available_content_width and the return value are
    ...
    """
    return min(
        max(min_content_width(context, box, outer=False), available_content_width),
...
def _block_content_width(context, box, function, outer):
    ...
    if width == "auto" or width.unit == "%":
        ...
        children_widths = [
            function(context, child, outer=True)
...
def margin_width(box, width, left=True, right=True):
    ...
    for value in (["margin_left", "padding_left"] if left else []) + (
...
def inline_min_content_width(
    context, box, outer=True, skip_stack=None, first_line=False, is_line_start=False
):
...
def table_cell_min_content_width(context, box, outer):
    ...
    children_min_width = adjust(
        box, outer, max(children_widths) if children_widths else 0
    )
...
def inline_line_widths(
    context, box, outer, is_line_start, minimum, skip_stack=None, first_line=False
):
        ...
        if isinstance(child, boxes.InlineBox):
            ...
            while new_resume_index is not None:
                ...
                _, _, new_resume_index, width, _, _ = split_first_line(
                    child_text[resume_index:].decode(),
                    child.style,
                    context,
                    max_width,
                    child.justification_spacing,
...
def _percentage_contribution(box):
    ...
    """
    min_width = (
        box.style["min_width"].value
        if box.style["min_width"] != "auto" and box.style["min_width"].unit == "%"
        else 0
    )
    max_width = (
        box.style["max_width"].value
        if box.style["max_width"] != "auto" and box.style["max_width"].unit == "%"
        else inf
    )
    width = (
        box.style["width"].value
        if box.style["width"] != "auto" and box.style["width"].unit == "%"
        else 0
    )
    return max(min_width, min(width, max_width))
...
def table_and_columns_preferred_widths(context, box, outer=True):
    ...
    if table.style["border_collapse"] == "separate" and grid_width > 0:
        total_horizontal_border_spacing = table.style["border_spacing"][0] * (
    ...
    for i in range(grid_width):
        if (
            column_groups[i]
            and column_groups[i].style["width"] != "auto"
            and column_groups[i].style["width"].unit != "%"
        ):
            constrainedness[i] = True
            continue
        if (
            columns[i]
            and columns[i].style["width"] != "auto"
            and columns[i].style["width"].unit != "%"
        ):
            constrainedness[i] = True
            continue
        for cell in zipped_grid[i]:
            if (
                cell
                and cell.colspan == 1
                and cell.style["width"] != "auto"
                and cell.style["width"].unit != "%"
            ):
                constrainedness[i] = True
                break
    ...
    table_max_content_width = total_horizontal_border_spacing + max(
        [
            sum(max_content_widths),
            large_percentage_contribution,
            *small_percentage_contributions,
        ]
    )
    ...
    table_min_content_width = max(
        table_min_content_width, adjust(table, outer=False, width=table_min_width)
    )
    table_max_content_width = max(
        table_max_content_width, adjust(table, outer=False, width=table_max_width)
    )
...
def replaced_max_content_width(box, outer=True):
    ...
    if width == "auto":
        ...
        if height == "auto" or height.unit == "%":
            ...
            height = height.value
        image = box.replacement
        intrinsic_width, intrinsic_height, intrinsic_ratio = image.get_intrinsic_size(
            box.style["image_resolution"], box.style["font_size"]
        )
        width, _ = default_image_sizing(
            intrinsic_width,
...
def flex_min_content_width(context, box, outer=True):
    ...
    if (
        ...
        and box.style["flex_wrap"] == "nowrap"
    ):
