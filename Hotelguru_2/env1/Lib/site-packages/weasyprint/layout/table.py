...
def table_layout(
        ...
        if not split_cells and not has_header:
            ...
            if horizontal_borders:
                table.border_top_width = (
                    max(width for _, (_, width, _) in horizontal_borders[skipped_rows])
                    / 2
                )
    ...
    def group_layout(group, position_y, bottom_space, page_is_empty, skip_stack):
        ...
        for index_row, row in enumerate(group.children[skip:], start=skip):
            ...
            if new_group_children:
                page_break = block_level_page_break(new_group_children[-1], row)
            ...
            if resume_at and not page_is_empty:
                ...
                # on its cells.
                avoid_break = avoid_page_break(
                    row.style["break_inside"], context
                ) or any(
            ...
            if ending_cells:  # in this row
                if row.height == "auto":
                    row_bottom_y = max(
                        cell.position_y + cell.border_height() for cell in ending_cells
                    )
                    row.height = max(row_bottom_y - row.position_y, 0)
                else:
                    row.height = max(
                        row.height,
                        max(row_cell.border_height() for row_cell in ending_cells),
                    )
        ...
        abort = (
            resume_at
            and not original_page_is_empty
            and (
                avoid_page_break(group.style["break_inside"], context)
        ...
        for row in group.children:
            if row.baseline is None:
                if row.children:
                    ...
                    row.baseline = (
                        max(cell.content_box_y() + cell.height for cell in row.children)
                        - row.position_y
                    )
                else:
                    row.baseline = 0
    ...
    table_rows = [
        child for child in table.children if not child.is_header and not child.is_footer
    ]
    ...
    def all_groups_layout():
        ...
        if header and footer:
            ...
            new_table_children, resume_at, next_page, end_position_y = (
                body_groups_layout(
                    skip_stack,
                    position_y + header_height,
                    bottom_space + footer_height,
        ...
        new_table_children, resume_at, next_page, end_position_y = body_groups_layout(
...
def fixed_table_layout(box):
    ...
    min_table_width = (
        sum(w for w in column_widths if w is not None) + all_border_spacing
    )
    columns_without_width = [i for i, w in enumerate(column_widths) if w is None]
...
def auto_table_layout(context, box, containing_block):
    ...
    guesses = (
        min_content_guess,
        min_content_percentage_guess,
        min_content_specified_guess,
        max_content_guess,
    )
...
def distribute_excess_width(
    ...
    columns = [
        i
        for i, _ in enumerate(grid[column_slice], start=column_slice.start)
        if not constrainedness[i]
        and column_intrinsic_percentages[i] == 0
        and column_max_content_widths[i] > 0
    ]
    if columns:
    ...
    columns = [
        i
        for i, column in enumerate(grid[column_slice], start=column_slice.start)
        if column
    ]
    if columns:
        for i in columns:
