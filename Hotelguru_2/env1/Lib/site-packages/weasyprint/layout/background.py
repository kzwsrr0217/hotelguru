...
def layout_box_backgrounds(
    page, box, get_image_from_uri, layout_children=True, style=None
):
...
def layout_background_layer(
    box, page, resolution, image, size, clip, repeat, origin, position, attachment
):
    ...
    if box is page:
        ...
        for row in box.children:
            if row.children:
                clipped_boxes += [cell.rounded_border_box() for cell in row.children]
                total_height = max(
                    total_height, max(cell.border_height() for cell in row.children)
                )
        painting_area = [
            box.border_box_x(),
            box.border_box_y(),
        ...
        if box.children:
            clipped_boxes = [cell.rounded_border_box() for cell in box.children]
        ...
        if cells:
            ...
            max_x = max(cell.border_box_x() + cell.border_width() for cell in cells)
            painting_area = [
                min_x,
                box.border_box_y(),
                max_x - min_x,
