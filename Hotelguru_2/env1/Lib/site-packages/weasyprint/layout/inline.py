    ...
    while True:
        ...
        if containing_block.style["direction"] == "ltr":
            ...
            condition = (position_x + line.width, position_y) == (
                original_position_x + original_width,
                original_position_y,
            )
        if condition:
...
def remove_last_whitespace(context, line):
    ...
    if not (
        ...
        and box.style["white_space"] in ("normal", "nowrap", "pre-line")
    ):
        return
...
def atomic_box(
    context, box, position_x, skip_stack, containing_block, absolute_boxes, fixed_boxes
):
...
def split_inline_level(
    ...
    if isinstance(box, boxes.TextBox):
        ...
        new_box, skip, preserved_line_break = split_text_box(
            context, box, max_x - position_x, skip, is_line_start=is_line_start
        )
...
def _out_of_flow_layout(
    ...
    if child.is_absolutely_positioned():
        ...
        non_floating_children = [
            child_
            for _, child_, _ in (children + waiting_children)
            if not child_.is_floated()
        ]
        if non_floating_children:
            float_width -= trailing_whitespace_size(context, non_floating_children[-1])
        ...
        if float_width > max_x - position_x or waiting_floats:
            ...
            for _, old_child in line_children:
                if not old_child.is_in_normal_flow():
                    continue
                if (
                    child.style["float"] == "left" and box.style["direction"] == "ltr"
                ) or (
                    child.style["float"] == "right" and box.style["direction"] == "rtl"
                ):
...
def split_inline_box(
    ...
    left_spacing = box.padding_left + box.margin_left + box.border_left_width
    right_spacing = box.padding_right + box.margin_right + box.border_right_width
    ...
    for i, child in enumerate(box.children[skip:]):
        ...
        if can_break is None:
            if None in (last_letter, first):
                ...
                can_break = can_break_text(last_letter + first, child.style["lang"])
        ...
        if new_child is None:
            ...
            trailing_whitespace = (
                isinstance(new_child, boxes.TextBox)
                and new_child.text
                and unicodedata.category(new_child.text[-1]) == "Zs"
            )
    ...
    if box.style["direction"] == "rtl" and len(children) > 1:
        in_flow_children = [
            box_child for _, box_child, _ in children if box_child.is_in_normal_flow()
        ]
        position_x = in_flow_children[0].position_x
        for child in in_flow_children[::-1]:
            child.translate(dx=(position_x - child.position_x), ignore_floats=True)
            position_x += child.margin_width()

    is_end = resume_at is None
    new_box = box.copy_with_children([box_child for index, box_child, _ in children])
    ...
    if isinstance(box, boxes.LineBox):
        ...
        for child in children:
            if child.is_in_normal_flow():
                new_box.width = (
                    child.position_x + child.margin_width() - new_box.position_x
                )
                break
    else:
        ...
        if box.style["box_decoration_break"] == "clone":
            ...
            translation_needed = is_start if box.style["direction"] == "ltr" else is_end
    ...
    new_box.margin_top = half_leading - new_box.border_top_width - new_box.padding_top
...
def split_text_box(context, box, available_width, skip, is_line_start=True):
    ...
    if resume_index is None:
        ...
        preserved_line_break = (length != resume_index) and between.strip(" ")
...
def line_box_verticality(box):
    ...
    """
    top_bottom_subtrees = []
    max_y, min_y = aligned_subtree_verticality(box, top_bottom_subtrees, baseline_y=0)
...
def is_phantom_linebox(linebox):
    ...
    for child in linebox.children:
        if isinstance(child, boxes.InlineBox):
            ...
            for side in ("top", "right", "bottom", "left"):
                if (
                    getattr(child.style[f"margin_{side}"], "value", None)
                    or child.style[f"border_{side}_width"]
                    or child.style[f"padding_{side}"].value
                ):
                    return False
