...
def columns_layout(
    ...
    for i, child in enumerate(box.children[skip:], start=skip):
        if child.style["column_span"] == "all":
            if column_children:
                columns_and_blocks.append((i - len(column_children), column_children))
            columns_and_blocks.append((i, child.copy()))
            column_children = []
            continue
        column_children.append(child.copy())
    if column_children:
        columns_and_blocks.append((i + 1 - len(column_children), column_children))
    ...
    for index, column_children_or_block in columns_and_blocks:
        if not isinstance(column_children_or_block, list):
            ...
            current_position_y = new_child.border_height() + new_child.border_box_y()
        ...
        while True:
            ...
            for i in range(count):
                ...
                in_flow_children = [
                    child for child in new_box.children if child.is_in_normal_flow()
                ]
                if in_flow_children:
                    # Get the empty space at the bottom of the column box
                    consumed_height = (
                        in_flow_children[-1].margin_height()
                        + in_flow_children[-1].position_y
                        - current_position_y
                    )
            ...
            if balancing:
                ...
                everything_fits = (
                    not column_skip_stack and max(consumed_heights) <= max_height
                )
                if everything_fits:
                    ...
                    if (
                        style["column_fill"] == "balance"
                        or index < columns_and_blocks[-1][0]
                    ):
                        balancing = True
        ...
        while True:
            ...
            new_child, column_skip_stack, column_next_page, _, _, _ = block_box_layout(
                context,
                column_box,
                bottom_space,
                skip_stack,
                containing_block,
                original_page_is_empty,
                absolute_boxes,
                fixed_boxes,
                None,
                discard=False,
            ...
            max_column_height = max(max_column_height, new_child.margin_height())
