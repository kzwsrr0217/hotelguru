...
def absolute_width(box, context, cb_x, cb_y, cb_width, cb_height):
    ...
    ltr = box.style.parent_style is None or box.style.parent_style["direction"] == "ltr"
    paddings_borders = (
        box.padding_left
        + box.padding_right
    ...
    if box.left == box.right == box.width == "auto":
        ...
        if box.left == box.width == "auto":
            box.width = shrink_to_fit(context, box, cb_width - spacing - box.right)
            ...
            box.width = shrink_to_fit(context, box, cb_width - spacing - box.left)
            translate_x = box.left + default_translate_x
        elif box.left == "auto":
            translate_x = (
                cb_width + default_translate_x - (box.right + spacing + box.width)
            )
        elif box.width == "auto":
...
def absolute_height(box, context, cb_x, cb_y, cb_width, cb_height):
    ...
    if box.top == box.bottom == box.height == "auto":
        ...
        if box.top == box.height == "auto":
            translate_y = cb_height - box.bottom - spacing + default_translate_y
            ...
            translate_y = (
                cb_height + default_translate_y - (box.bottom + spacing + box.height)
            )
...
def absolute_layout(
    context, placeholder, containing_block, fixed_boxes, bottom_space, skip_stack
):
...
def absolute_box_layout(
    ...
    if isinstance(box, boxes.BlockReplacedBox):
        new_box = absolute_replaced(context, box, cb_x, cb_y, cb_width, cb_height)
        resume_at = None
    else:
