...
def block_box_layout(
    ...
    if box.style["column_width"] != "auto" or box.style["column_count"] != "auto":
        ...
        if resume_at is None:
            ...
            if columns_bottom_space:
                remove_placeholders(context, [new_box], absolute_boxes, fixed_boxes)
...
def block_level_width(box, containing_block):
    ...
    if width == "auto":
        ...
        if margin_r == "auto":
            margin_r = box.margin_right = 0
        width = box.width = cb_width - (paddings_plus_borders + margin_l + margin_r)
...
def _out_of_flow_layout(
    ...
    # Float child layout.
        ...
        add_child = (
            ...
            or not page_overflow
            or box.is_monolithic()
        )
        if add_child:
...
def _break_line(
    ...
    if needed and needed <= over_orphans:
        ...
        for child in new_children[-needed:]:
            remove_placeholders(context, child.children, absolute_boxes, fixed_boxes)
...
def _linebox_layout(
    ...
    for i, (line, resume_at) in enumerate(lines_iterator):
        ...
        overflow = (new_children or not page_is_empty) and context.overflows_page(
        ...
        if context.footnotes:
            ...
            for footnote in footnotes:
                ...
                overflow = (
                    overflow
                    or context.reported_footnotes
                    or context.overflows_page(bottom_space, new_position_y + offset_y)
                )
                if overflow:
...
def _in_flow_layout(
    ...
    if not new_containing_block.is_table_wrapper:
        ...
        if last_in_flow_child is None and collapsing_with_children:
            ...
            collapsed_margin_difference = new_collapsed_margin - old_collapsed_margin
    ...
    page_is_empty_with_no_children = page_is_empty and not any(
        child for child in new_children if not isinstance(child, AbsolutePlaceholder)
    )
...
def block_container_layout(
    ...
    collapsing_with_children = not (
        ...
        or box.is_flex_item
        or box.is_grid_item
        or box.establishes_formatting_context()
        or box.is_for_root_element
    )
    ...
    for index, child in enumerate(box.children[skip:], start=(skip or 0)):
        ...
        if not child.is_in_normal_flow():
            ...
            if out_of_flow_resume_at:
                broken_out_of_flow[new_child] = (child, box, out_of_flow_resume_at)

        elif isinstance(child, boxes.LineBox):
            (abort, stop, resume_at, position_y, new_footnotes, max_lines) = (
                _linebox_layout(
                    context,
                    box,
                    index,
        ...
        if abort:
            ...
            return (None, None, {"break": "any", "page": page}, [], False, max_lines)
        elif stop:
            if box.height != "auto":
    ...
    if (
        box_is_fragmented
        and avoid_page_break(box.style["break_inside"], context)
    ...
    if last_in_flow_child is None:
        ...
        if (
            box.height in ("auto", 0)
            and get_clearance(context, box, collapsed_margin) is None
            and all(
                value == 0
                for value in (
                    box.min_height,
    ...
    if (
        box.border_bottom_width
        or box.padding_bottom
        or box.establishes_formatting_context()
        or box.is_for_root_element
        or box.is_table_wrapper
    ):
        position_y += collapse_margin(adjoining_margins)
        adjoining_margins = []
    ...
    new_box.remove_decoration(start=not is_start, end=box_is_fragmented and not discard)
...
def force_page_break(page_break, context):
    ...
    if context.in_column:
        return page_break in ("page", "left", "right", "recto", "verso", "column")
