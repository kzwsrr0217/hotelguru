...
def _intersect(position_1, size_1, position_2, size_2):
    return position_1 < position_2 + size_2 and position_2 < position_1 + size_1
...
def _get_second_placement(
    ...
    if dense:
        ...
        if second_start == "auto":
            return _get_placement((None, track + 1, None), second_end, second_tracks)
        else:
            assert second_start[0] == "span"
...
def _distribute_extra_space(
    ...
    for functions in sizing_functions:
        ...
        if affected_tracks_types == "intrinsic":
            if (
                function in ("min-content", "max-content", "auto")
                or function[0] == "fit-content()"
            ):
    ...
    for i, children in enumerate(tracks_children):
        ...
        for item in children:
            ...
            affected_tracks_numbers = [j for j, affected in tracks_numbers if affected]
...
def _resolve_tracks_sizes(
    ...
    for min_function, max_function in sizing_functions:
        ...
        if _is_length(min_function):
            ...
            min_function in ("min-content", "max-content", "auto")
            or min_function[0] == "fit-content()"
        ):
            base_size = 0
        growth_limit = None
        if _is_length(max_function):
            ...
            max_function in ("min-content", "max-content", "auto")
            or max_function[0] == "fit-content()"
            or _is_fr(max_function)
        ):
            growth_limit = inf
    ...
    for children, (min_function, max_function), sizes in iterable:
        ...
        if min_function == "min-content":
            sizes[0] = max(
                0, *(min_content_width(context, child) for child in children)
            )
        elif min_function == "max-content":
            sizes[0] = max(
                0, *(max_content_width(context, child) for child in children)
            )
        ...
        if max_function == "min-content":
            ...
            max_function in ("auto", "max-content")
            or max_function[0] == "fit_content()"
        ):
        ...
        for sizes, (_, max_function) in iterable:
            if _is_fr(max_function):
                if max_function.value > 1:
                    flex_fraction = max(flex_fraction, max_function.value * sizes[0])
                else:
    ...
    if (x_stretch or y_stretch) and free_space is not None and free_space > 0:
        auto_tracks_sizes = [
            sizes
            for sizes, (min_function, _) in zip(tracks_sizes, sizing_functions)
...
def grid_layout(
        ...
        for child in remaining_grid_items:
            ...
            if second_placement:
                ...
                for cursor_first in count(cursor_first):
                    if first_start == "auto":
                        first_i, first_size = _get_placement(
                            (None, cursor_first + 1, None), first_end, first_tracks[::2]
                        )
                    else:
                        assert first_start[0] == "span"
                        span = _get_span(first_start)
    ...
    rows_sizes = _resolve_tracks_sizes(
        ...
        context,
        box,
        [size for size, _ in columns_sizes],
    )
    ...
    row_lines_positions = rows_positions[skip_row + 1 :] + [
        box.content_box_y() + total_height
    ]
    ...
    for child in this_page_children:
        ...
        height = (
            sum(size for size, _ in rows_sizes[y : y + height]) + (height - 1) * row_gap
        )
