...
def find_stylesheets(
    ...
    for wrapper in wrapper_element.query_all("style", "link"):
        ...
        if element.tag == "style":
            ...
            if not element_has_link_type(
                element, "stylesheet"
            ) or element_has_link_type(element, "alternate"):
                continue
...
class ComputedStyle(dict):
    ...
    def __init__(
        self, parent_style, cascaded, element, pseudo_type, root_style, base_url
    ):
        self.specified = {}
        self.parent_style = parent_style
...
def preprocess_stylesheet(
    ...
    for rule in stylesheet_rules:
        ...
        if rule.type == "qualified-rule":
            try:
                ...
                if selectors_declarations:
                    ...
                    for selectors, declarations in selectors_declarations:
                        declarations = [declaration[1] for declaration in declarations]
                ...
                for margin_rule in content:
                    ...
                    if declarations:
                        selector_list = [
                            (
                                specificity,
                                f"@{margin_rule.lower_at_keyword}",
                                page_selector_type,
                            )
                        ]
            ...
            LOGGER.warning(
                "Unknown rule %s at %d:%d", rule, rule.source_line, rule.source_column
            )
