...
def compute_position(style, name, values):
    ...
    return tuple(
        (origin_x, length(style, name, pos_x), origin_y, length(style, name, pos_y))
...
def background_size(style, name, values):
    ...
    return tuple(
        (
            ...
            else length_or_percentage_tuple(style, name, value)
        )
        for value in values
    )
...
def _content_list(style, values):
    ...
    for value in values:
        ...
        if computed_value is None:
            LOGGER.warning(
                "Unable to compute %r value for content: %r"
...
def string_set(style, name, values):
    ...
    return tuple(
        (string_set[0], _content_list(style, string_set[1])) for string_set in values
    )
    ...
    for i, value in enumerate(values):
        if i % 2 == 0:
            ...
            if track_breadth:
                ...
                return_values.append(("fit-content()", length(style, name, value[1])))
            elif value[0] == "repeat()":
                return_values.append(
    ...
    for value in values:
        ...
        if track_breadth:
            ...
            return_values.append(
                (
                    "minmax()",
                    grid_auto(style, name, [value[1]])[0],
