...
LENGTHS_TO_PIXELS = {
    "px": 1,
    "pt": 1.0 / 0.75,
    "pc": 16.0,  # LENGTHS_TO_PIXELS['pt'] * 12
    "in": 96.0,  # LENGTHS_TO_PIXELS['pt'] * 72
    "cm": 96.0 / 2.54,  # LENGTHS_TO_PIXELS['in'] / 2.54
    "mm": 96.0 / 25.4,  # LENGTHS_TO_PIXELS['in'] / 25.4
    "q": 96.0 / 25.4 / 4,  # LENGTHS_TO_PIXELS['mm'] / 4
}
...
def remove_whitespace(tokens):
    ...
    return tuple(
        token for token in tokens if token.type not in ("whitespace", "comment")
    )
...
def parse_2d_position(tokens):
    ...
    elif keyword_1 in ("left", "center", "right") and keyword_2 in (
        "top",
        "center",
        "bottom",
    ):
    ...
    elif keyword_1 in ("top", "center", "bottom") and keyword_2 in (
        "left",
        "center",
        "right",
    ):
...
def parse_position(tokens):
    ...
    if len(tokens) == 4:
        ...
        if length_1 and length_2:
            if keyword_1 in ("left", "right") and keyword_2 in ("top", "bottom"):
                return keyword_1, length_1, keyword_2, length_2
            if keyword_2 in ("left", "right") and keyword_1 in ("top", "bottom"):
    ...
    if len(tokens) == 3:
        ...
        if length is not None:
            ...
            elif keyword in ("left", "right") and other_keyword in ("top", "bottom"):
                return keyword, length, other_keyword, ZERO_PERCENT
            elif keyword in ("top", "bottom") and other_keyword in ("left", "right"):
...
def parse_radial_gradient_parameters(arguments):
    ...
    while stack:
        ...
        if keyword == "at":
            ...
            keyword
            in ("closest-corner", "farthest-corner", "closest-side", "farthest-side")
            and size is None
        ):
...
def get_quote(token):
    """Parse a <quote> token."""
    keyword = get_keyword(token)
    if keyword in ("open-quote", "close-quote", "no-open-quote", "no-close-quote"):
        return keyword
