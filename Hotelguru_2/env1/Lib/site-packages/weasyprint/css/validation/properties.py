...
def property(
    property_name=None, proprietary=False, unstable=False, wants_base_url=False
):
...
def background_repeat(tokens):
    ...
    if len(keywords) == 2 and all(
        k in ("no-repeat", "repeat", "space", "round") for k in keywords
    ):
        return keywords
...
def content(tokens, base_url):
    ...
    while tokens:
        if len(tokens) >= 2 and (
            ...
            parsed_token = get_image(token, base_url) or get_url(token, base_url)
            if parsed_token:
    ...
    if (
        ...
        and (tokens[-2].type == "literal" and tokens[-2].value == "/")
    ):
    ...
    def font_feature_settings_list(tokens):
        ...
        if len(tokens) == 2:
            ...
            if token.type == "ident":
                ...
                token.type == "number"
                and token.int_value is not None
                and token.int_value >= 0
            ):
...
def quotes(tokens):
    ...
    if (
        tokens
        and len(tokens) % 2 == 0
        and all(token.type == "string" for token in tokens)
    ):
    ...
    if function:
        ...
        if name == "minmax" and len(args) == 2:
            ...
            if keyword in ("min-content", "max-content", "auto") or length:
                fixed_breadth = get_length(args[1], negative=False, percentage=True)
                if fixed_breadth:
...
def align_content(tokens):
    ...
    if len(tokens) == 1:
        ...
        keywords = tuple(get_keyword(token) for token in tokens)
        if keywords[0] in ("safe", "unsafe"):
            if keywords[1] in ("center", "start", "end", "flex-start", "flex-end"):
                return keywords
        elif "baseline" in keywords:
            if "first" in keywords or "last" in keywords:
                return keywords
...
def hyphenate_limit_chars(tokens):
    ...
    if len(tokens) == 1:
        ...
        if (
            (
                get_keyword(total) == "auto"
                or (total.type == "number" and total.int_value is not None)
            )
            and (
                get_keyword(left) == "auto"
                or (left.type == "number" and left.int_value is not None)
            )
            and (
                get_keyword(right) == "auto"
                or (right.type == "number" and right.int_value is not None)
            )
        ):
    ...
    if get_single_keyword(tokens) == "none":
        ...
        for token in tokens:
            ...
            if len(args) == 1:
                ...
                len(args) == 6
                and name == "matrix"
                and all(a.type == "number" for a in args)
            ):
                transforms.append((name, tuple(arg.value for arg in args)))
            else:
                return
