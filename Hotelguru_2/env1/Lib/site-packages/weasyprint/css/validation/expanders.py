...
@generic_expander(
    "-outset", "-repeat", "-slice", "-source", "-width", wants_base_url=True
)
...
@generic_expander(
    "-outset", "-repeat", "-slice", "-source", "-width", "-mode", wants_base_url=True
)
...
def expand_background(tokens, name, base_url):
    ...
    expander = functools.partial(expand_background, name=name, base_url=base_url)
    if result := _find_var(tokens, expander, expanded_names):
        yield from result.items()
        return
    ...
    def parse_layer(tokens, final_layer=False):
        ...
        color = results.pop("background-color", INITIAL_VALUES["background_color"])
        for name in expanded_names:
...
@generic_expander(
    "-alternates", "-caps", "-east-asian", "-ligatures", "-numeric", "-position"
)
    ...
    if keyword == "none":
        ...
        for token in tokens:
            ...
            forced_flex_factor = (
                ...
                and not all((grow_found, shrink_found))
            )
...
def expand_grid(tokens, name):
    ...
    yield "-auto-flow", ((auto_track_token, dense) if dense else (auto_track_token,))
