    ...
    for descriptor in descriptors:
        ...
        try:
            ...
            LOGGER.warning(
                ...
                descriptor.name,
                tinycss2.serialize(descriptor.value),
...
def expand_font_variant(tokens):
    ...
    if keyword in ("normal", "none"):
        for suffix in ("-alternates", "-caps", "-east-asian", "-numeric", "-position"):
...
def font_family(tokens, allow_spaces=False):
    ...
    if tokens and all(token.type in allowed_types for token in tokens):
        return " ".join(token.value for token in tokens if token.type == "ident")
...
def font_variant(tokens):
    ...
    for name, sub_tokens in expand_font_variant(tokens):
        try:
            values.append(
                properties.validate_non_shorthand(
