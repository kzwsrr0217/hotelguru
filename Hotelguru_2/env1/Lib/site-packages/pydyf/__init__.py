...
class Stream(Object):
    ...
    def curve_start_to(self, x2, y2, x3, y3):
        ...
        self.stream.append(
            b" ".join(
                (_to_bytes(x2), _to_bytes(y2), _to_bytes(x3), _to_bytes(y3), b"v")
            )
        )
    ...
    def curve_end_to(self, x1, y1, x3, y3):
        ...
        self.stream.append(
            b" ".join(
                (_to_bytes(x1), _to_bytes(y1), _to_bytes(x3), _to_bytes(y3), b"y")
            )
        )
    ...
    def rectangle(self, x, y, width, height):
        ...
        self.stream.append(
            b" ".join(
                (_to_bytes(x), _to_bytes(y), _to_bytes(width), _to_bytes(height), b"re")
            )
        )
    ...
    def set_color_rgb(self, r, g, b, stroke=False):
        ...
        self.stream.append(
            b" ".join(
                (_to_bytes(r), _to_bytes(g), _to_bytes(b), (b"RG" if stroke else b"rg"))
            )
        )
    ...
    def set_color_special(self, name, stroke=False, *operands):
        ...
        self.stream.append(
            b" ".join(_to_bytes(operand) for operand in operands)
            + b" "
    ...
    def set_dash(self, dash_array, dash_phase):
        ...
        """
        self.stream.append(
            b" ".join((Array(dash_array).data, _to_bytes(dash_phase), b"d"))
        )

    def set_font_size(self, font, size):
        """Set font name and size."""
        self.stream.append(b"/" + _to_bytes(font) + b" " + _to_bytes(size) + b" Tf")
...
class PDF:
    ...
    def write(self, output, version=b"1.7", identifier=False, compress=False):
        ...
        if version >= b"1.5" and compress:
            ...
            max_generation = max(object_.generation for object_ in self.objects)
            field3_size = ceil(
                log(max(max_generation, len(compressed_objects)) + 1, 256)
            )
            xref_lengths = (1, field2_size, field3_size)
            xref_stream = b"".join(
                value.to_bytes(length, "big")
                for line in xref
                for length, value in zip(xref_lengths, line)
            )
            extra = {
                "Type": "/XRef",
