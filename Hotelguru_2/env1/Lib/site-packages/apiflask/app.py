    ...
    *Version changed: 0.9.0*
    ...
    def _register_openapi_blueprint(self) -> None:
        ...
        *Version changed: 0.7.0*
        ...
        """
        bp = Blueprint(
            "openapi", __name__, url_prefix=self.openapi_blueprint_url_prefix
        )
            ...
            if self.docs_ui == "swagger-ui":
                if self.docs_oauth2_redirect_path:
                    ...
                    @self._apply_decorators(
                        config_name="SWAGGER_UI_OAUTH_REDIRECT_DECORATORS"
                    )
    ...
    def _get_spec(
        self, spec_format: str | None = None, force_update: bool = False
    ) -> dict | str:
        ...
        for rule in rules:
            ...
            for method in ["GET", "POST", "PUT", "PATCH", "DELETE"]:
                ...
                if hasattr(view_func, "_method_spec"):
                    ...
                    if (
                        view_func._spec.get("no_spec")
                        and not self.config["AUTO_200_RESPONSE"]
                    ):
                        continue
                    if (
                ...
                if view_func._spec.get("description"):
                    ...
                    if self.config["AUTO_OPERATION_DESCRIPTION"]:
                        docs = [
                            line.strip()
                            for line in (view_func.__doc__ or "").strip().split("\n")
                        ]
                        if len(docs) > 1:
                ...
                if view_func._spec.get("response"):
                    ...
                    if (
                        not view_func._spec.get("responses")
                        and self.config["AUTO_200_RESPONSE"]
                    ):
                        self._add_response(
                            operation,
                            "200",
                ...
                has_bp_level_auth = (
                    ...
                    and blueprint_name in self._auth_blueprints
                )
                ...
                if view_func._spec.get("responses"):
                    ...
                    if isinstance(responses, list):
                        ...
                        for status_code in view_func._spec.get("responses"):
                            responses[status_code] = get_reason_phrase(  # type: ignore
                                int(status_code), ""
                            )
                    for status_code, value in responses.items():  # type: ignore
                        ...
                        if isinstance(value, dict):
                            existing_response = operation["responses"].setdefault(
                                status_code, {}
                            )
                            existing_response_content = existing_response.setdefault(
                                "content", {}
                            )
                            existing_response_content.update(value.get("content", {}))
                            if (
                                new_description := value.get("description")
                            ) is not None:
                ...
                if custom_security:  # custom security
                    ...
                    if isinstance(operation_security, str):  # 'A' -> [{'A': []}]
                        ...
                        if isinstance(operation_security[0], str):
                            operation["security"] = [
                                {name: []} for name in operation_security
                            ]
                        else:
                            operation["security"] = operation_security
                    else:
