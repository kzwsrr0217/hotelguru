        ...
        if node is not Marker:
            for element in self[::-1]:
                if element is Marker:
                    break
                nodes_equal = (
                    element.name_tuple == node.name_tuple
                    and element.attributes == node.attributes
                )
                if nodes_equal:
                    equal_count += 1
                if equal_count == 3:
...
class TreeBuilder:
    ...
    def insert_text(self, data, parent=None):
        ...
        in_table = (
            ...
            and self.open_elements[-1].name in table_insert_mode_elements
        )
        if in_table:
            # We should be in the InTable mode. This means we want to do
            # special magic element rearranging.
    ...
    def get_document(self, full_tree=False):
        ...
        return self.document._element.find(
            ...
            else f"{{{self.default_namespace}}}html"
        )
