...
class HTMLTokenizer:
    ...
    def consume_number_entity(self, is_hex):
        ...
        if integer in replacement_characters:
            ...
            if (
                (0x0001 <= integer <= 0x0008)
                or (0x000E <= integer <= 0x001F)
                or (0x007F <= integer <= 0x009F)
                or (0xFDD0 <= integer <= 0xFDEF)
                or integer
                in frozenset(
                    [
                        0x000B,
    ...
    def consume_entity(self, allowed=None, from_attribute=False):
        ...
        unget = stack[0] in (EOF, "<", "&", *space_characters) or (
        ...
        if unget:
            ...
            try:
                ...
                allowed_character = (
                    stack[entity_length] in ascii_letters
                    or stack[entity_length] in digits
                    or stack[entity_length] == "="
                )
    ...
    def rcdata_end_tag_name_state(self):
        appropriate = (
            self.current_token
            and self.current_token["name"].lower() == self.temporary_buffer.lower()
        )
        data = self.stream.character()
    ...
    def attribute_name_state(self):
        ...
        if data == "=":
            ...
            self.current_token["data"][-1][0] += data + self.stream.chars_until(
                ascii_letters, True
            )
        ...
        if leaving_this_state:
            ...
            self.current_token["data"][-1][0] = self.current_token["data"][-1][
                0
            ].translate(ascii_upper_to_lower)
    ...
    def markup_declaration_open_state(self):
        ...
        if stack[-1] == "-":
            ...
            and self.parser.tree.open_elements
            and self.parser.tree.open_elements[-1].namespace
    ...
    def comment_state(self):
        ...
        if data == "-":
            ...
            self.current_token["data"] += data + self.stream.chars_until(
                ("-", "\u0000")
            )
        return True
