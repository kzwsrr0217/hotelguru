    ...
    def is_html_integration_point(self, element):
        ...
        if full_name == (namespaces["mathml"], "annotation-xml"):
            return "encoding" in element.attributes and element.attributes[
                "encoding"
            ].translate(ascii_upper_to_lower) in ("text/html", "application/xhtml+xml")
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == Token.PARSE_ERROR:
                    ...
                    if (
                        len(self.tree.open_elements) == 0
                        or current_node_namespace == self.tree.default_namespace
                        or (
                            self.is_mathml_text_integration_point(current_node)
                            and (
                                (
                                ...
                                or type in (Token.CHARACTERS, Token.SPACE_CHARACTERS)
                            )
                        )
                        or (
                            current_node_namespace == namespaces["mathml"]
                            and current_node_name == "annotation-xml"
                            and type == Token.START_TAG
                            and token["name"] == "svg"
                        )
                        or (
                            self.is_html_integration_point(current_node)
                            and type
                            in (
                                Token.START_TAG,
                                Token.CHARACTERS,
            ...
            if (
                type == Token.START_TAG
                and previous_token["selfClosing"]
                and not previous_token["selfClosingAcknowledged"]
            ):
                self.parse_error(
...
class InitialPhase(Phase):
    ...
    def process_doctype(self, token):
        ...
        if (
            ...
            or system_id is not None
            and system_id != "about:legacy-compat"
        ):
...
class BeforeHeadPhase(Phase):
    ...
    start_tag_handler = dispatch([("html", start_tag_html), ("head", start_tag_head)])

    end_tag_handler = dispatch([(("head", "body", "html", "br"), end_tag_imply_head)])
...
class InHeadPhase(Phase):
    ...
    end_tag_handler = dispatch(
        [("head", end_tag_head), (("br", "html", "body"), end_tag_html_body_br)]
    )
...
class AfterHeadPhase(Phase):
    ...
    end_tag_handler = dispatch([(("body", "html", "br"), end_tag_html_body_br)])
...
class InBodyPhase(Phase):
    ...
    def process_space_characters_drop_newline(self, token):
        ...
        if (
            data.startswith("\n")
            and self.tree.open_elements[-1].name in ("pre", "listing", "textarea")
            and not self.tree.open_elements[-1].has_content()
        ):
            data = data[1:]
        if data:
    ...
    def start_tag_body(self, token):
        ...
        if (
            len(self.tree.open_elements) == 1
            or self.tree.open_elements[1].name != "body"
        ):
    ...
    def start_tag_list_item(self, token):
        ...
        for node in reversed(self.tree.open_elements):
            if node.name in stop_names:
                self.parser.phase.process_end_tag(implied_tag_token(node.name))
                break
            if node.name_tuple in special_elements and node.name not in (
                "address",
                "div",
                "p",
            ):
    ...
    def start_tag_input(self, token):
        ...
        if (
            "type" in token["data"]
            and token["data"]["type"].translate(ascii_upper_to_lower) == "hidden"
        ):
    ...
    def end_tag_formatting(self, token):
        ...
        while outer_loop_counter < 8:
            ...
            if not formatting_element or (
                formatting_element in self.tree.open_elements
                and not self.tree.element_in_scope(formatting_element.name)
            ):
            ...
            while inner_loop_counter < 3:
                ...
                self.tree.active_formatting_elements[
                    self.tree.active_formatting_elements.index(node)
                ] = clone
...
class InColumnGroupPhase(Phase):
    ...
    start_tag_handler = dispatch(
        [("html", Phase.start_tag_html), ("col", start_tag_col)]
    )

    end_tag_handler = dispatch([("colgroup", end_tag_colgroup), ("col", end_tag_col)])
...
class InTableBodyPhase(Phase):
    ...
    def start_tag_table_other(self, token):
        ...
        if (
            self.tree.element_in_scope("tbody", variant="table")
            or self.tree.element_in_scope("thead", variant="table")
            or self.tree.element_in_scope("tfoot", variant="table")
        ):
            self._clear_stack_to_table_body_context()
...
class InCellPhase(Phase):
    ...
    def start_tag_table_other(self, token):
        if self.tree.element_in_scope(
            "td", variant="table"
        ) or self.tree.element_in_scope("th", variant="table"):
            self._close_cell()
            return token
        else:
            # Fragment case.
...
class InSelectPhase(Phase):
    ...
    def end_tag_optgroup(self, token):
        ...
        if (
            self.tree.open_elements[-1].name == "option"
            and self.tree.open_elements[-2].name == "optgroup"
        ):
            self.tree.open_elements.pop()
...
class InForeignContentPhase(Phase):
    ...
    def process_characters(self, token):
        if token["data"] == "\u0000":
            token["data"] = "\ufffd"
        elif self.parser.frameset_ok and any(
            char not in space_characters for char in token["data"]
        ):
            self.parser.frameset_ok = False
    ...
    def process_start_tag(self, token):
        ...
        if token["name"] in self.breakout_elements or (
            ...
            while (
                self.tree.open_elements[-1].namespace != self.tree.default_namespace
                and not self.parser.is_html_integration_point(
                    self.tree.open_elements[-1]
                )
                and not self.parser.is_mathml_text_integration_point(
                    self.tree.open_elements[-1]
                )
            ):
                self.tree.open_elements.pop()
...
class AfterBodyPhase(Phase):
    ...
    start_tag_handler = dispatch([("html", start_tag_html)])
...
class InFramesetPhase(Phase):
    ...
    def end_tag_frameset(self, token):
        if self.tree.open_elements[-1].name == "html":
            ...
            self.tree.open_elements.pop()
        if not self.parser.container and self.tree.open_elements[-1].name != "frameset":
    ...
    end_tag_handler = dispatch([("frameset", end_tag_frameset)])
...
class AfterFramesetPhase(Phase):
    ...
    start_tag_handler = dispatch(
        [("html", Phase.start_tag_html), ("noframes", start_tag_noframes)]
    )
...
class AfterAfterBodyPhase(Phase):
    ...
    def process_end_tag(self, token):
        self.parser.parse_error("expected-eof-but-got-end-tag", {"name": token["name"]})
        self.parser.phase = self.parser.phases["in body"]
        return token
...
class AfterAfterFramesetPhase(Phase):
    ...
    start_tag_handler = dispatch(
        [("html", start_tag_html), ("noframes", start_tag_noframes)]
    )
