...
class MigrationContext:
    ...
    def __init__(
        ...
        if as_sql:
            ...
            self._in_external_transaction = sqla_compat._get_connection_in_transaction(
                connection
            )

        self._migrations_fn: Optional[Callable[..., Iterable[RevisionStep]]] = opts.get(
            "fn"
        )
        self.as_sql = as_sql
        ...
        if "output_encoding" in opts:
            self.output_buffer = EncodedIO(
                opts.get("output_buffer") or sys.stdout,  # type:ignore[arg-type]
    ...
    def autocommit_block(self) -> Iterator[None]:
        ...
        if not self.as_sql:
            ...
            # place.
            self.connection = self.impl.connection = base_connection.execution_options(
    ...
    def get_current_heads(self) -> Tuple[str, ...]:
        ...
        return tuple(
            row[0]
            for row in self.connection.execute(select(self._version.c.version_num))
        )
