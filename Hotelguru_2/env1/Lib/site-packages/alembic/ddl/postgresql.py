...
class PostgresqlImpl(DefaultImpl):
    ...
    def correct_for_autogen_constraints(
        ...
        doubled_constraints = {
            index for index in conn_indexes if index.info.get("duplicates_constraint")
        }
...
class CreateExcludeConstraintOp(ops.AddConstraintOp):
    ...
    def from_constraint(  # type:ignore[override]
        ...
        return cls(
            ...
            [(expr, op) for expr, name, op in constraint._render_exprs],  # type: ignore
...
def _exclude_constraint(
    ...
    def do_expr_where_opts():
        ...
        if constraint.where is not None:
            args.append(
                "where=%s"
                % render._render_potential_expr(constraint.where, autogen_context)
            )
...
def _render_potential_column(
    value: Union[ColumnClause[Any], Column[Any], TextClause, FunctionElement[Any]],
    autogen_context: AutogenContext,
) -> str:
