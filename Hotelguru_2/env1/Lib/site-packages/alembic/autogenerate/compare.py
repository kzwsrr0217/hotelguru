...
def _compare_columns(
    ...
    metadata_col_names = OrderedSet(c.name for c in metadata_table.c if not c.system)
    metadata_cols_by_name = {c.name: c for c in metadata_table.c if not c.system}
...
def _compare_indexes_and_uniques(
    ...
    if conn_table is not None:
        ...
        try:
            conn_uniques = (
                inspector.get_unique_constraints(  # type:ignore[assignment] # noqa
                    tname, schema=schema
                )
            )
        ...
        conn_indexes = {  # type:ignore[assignment]
            index
            for index in (_make_index(impl, ix, conn_table) for ix in conn_indexes)
    ...
    metadata_unique_constraints_sig = {
        impl._create_metadata_constraint_sig(uq) for uq in metadata_unique_constraints
    }
...
def _correct_for_uq_duplicates_uix(
    ...
    for overlap in uqs_dupe_indexes:
        if overlap not in metadata_uq_names:
            if (
                impl._create_reflected_constraint_sig(uqs_dupe_indexes[overlap]).unnamed
