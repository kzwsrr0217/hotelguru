...
def _render_create_table_comment(
    ...
    return templ.format(
        ...
        existing=(
            "%r" % op.existing_comment if op.existing_comment is not None else None
        ),
...
def _add_table(autogen_context: AutogenContext, op: ops.CreateTableOp) -> str:
    ...
    args = [
        col
        for col in [_render_column(col, autogen_context) for col in table.columns]
        if col
    ] + sorted(
        [
            rcons
            for rcons in [
                _render_constraint(cons, autogen_context, op._namespace_metadata)
...
def _add_index(autogen_context: AutogenContext, op: ops.CreateIndexOp) -> str:
    ...
    text = tmpl % {
        ...
        "schema": (
            (", schema=%r" % _ident(index.table.schema)) if index.table.schema else ""
        ),
    ...
    *,
    ...
    if isinstance(value, sql.ClauseElement):
        ...
        if wrap_in_element:
            ...
            if is_index and isinstance(value, Label) and type(value.name) is str:
...
def _render_server_default(
    default: Optional[Union[FetchedValue, str, TextClause, ColumnElement[Any]]],
    autogen_context: AutogenContext,
    repr_: bool = True,
) -> Optional[str]:
...
def _render_ARRAY_type(type_: ARRAY, autogen_context: AutogenContext) -> str:
    return cast(
        str,
        _render_type_w_subtype(type_, autogen_context, "item_type", r"(.+?\()"),
    )
...
def _render_foreign_key(
    ...
    return "%(prefix)sForeignKeyConstraint([%(cols)s], " "[%(refcols)s], %(args)s)" % {
        "prefix": _sqlalchemy_autogenerate_prefix(autogen_context),
        "cols": ", ".join(repr(_ident(f.parent.name)) for f in constraint.elements),
        "refcols": ", ".join(
