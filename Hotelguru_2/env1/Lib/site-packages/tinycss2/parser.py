...
def _consume_blocks_content(first_token, tokens):
    ...
    if first_token != ";" and first_token.type != "{} block":
        for token in tokens:
            ...
            if token.type == "{} block":
                break
    declaration = _parse_declaration(first_token, iter(declaration_tokens), nested=True)
...
def _consume_qualified_rule(first_token, tokens, nested=False, stop_token=None):
