...
class HTTPAuth(object):
    ...
    def is_compatible_auth(self, headers):
        if self.header is None or self.header == "Authorization":
            try:
                scheme, _ = request.headers.get("Authorization", "").split(None, 1)
            except ValueError:
    ...
    def get_auth_password(self, auth):
        ...
        if auth and auth.username:
            password = self.ensure_sync(self.get_password_callback)(auth.username)
            ...
            def decorated(*args, **kwargs):
                ...
                if request.method != "OPTIONS":  # pragma: no cover
                    ...
                    g.flask_httpauth_user = (
                        user if user is not True else auth.username if auth else None
                    )
...
class HTTPBasicAuth(HTTPAuth):
    ...
    def get_auth(self):
        ...
        return Authorization(scheme, {"username": username, "password": password})
    ...
    def authenticate(self, auth, stored_password):
        ...
        if self.hash_password_callback:
            try:
                client_password = self.ensure_sync(self.hash_password_callback)(
                    client_password
                )
            except TypeError:
                client_password = self.ensure_sync(self.hash_password_callback)(
        ...
        return (
            ...
            and hmac.compare_digest(client_password, stored_password)
            else None
        )
...
class HTTPDigestAuth(HTTPAuth):
    def __init__(
        self, scheme=None, realm=None, use_ha1_pw=False, qop="auth", algorithm="MD5"
    ):
    ...
    def authenticate_header(self):
        ...
        if self.qop:
            return (
                '{0} realm="{1}",nonce="{2}",opaque="{3}",algorithm="{4}"' ',qop="{5}"'
            ).format(
                self.scheme,
                self.realm,
                nonce,
    ...
    def authenticate(self, auth, stored_password_or_ha1):
        if (
            ...
            return False
        if not self.verify_nonce_callback(
            auth.nonce
        ) or not self.verify_opaque_callback(auth.opaque):
            return False
        ...
        if self.algorithm == "MD5-Sess":
            ha1 = md5(
                (ha1 + ":" + auth.nonce + ":" + auth.cnonce).encode("utf-8")
            ).hexdigest()
